...
2. A HARK consumer type — Provides `solve()`, `simulate()`,
...
- `"WarmGlow" in agent_name` → compute `BeqFac`/`BeqShift` from
  `BeqMPC`/`BeqInt`
...
`content/tables/{min,msm,TRP}/`. On subsequent runs, `get_initial_guess()` reads
these files to warm-start the optimizer.
