* This file selects variables, using SCF95
...
gen YEAR = 1995                /* Indicates data is from which wave */
...
scalar CPIADJ = CPIBASE/2265           /* Adjust with CPI (adjusted to 1992$ price) */
scalar CPILAG = 2254/2201              /* Income is the previous year's income level, CPILAG adjust income to survey year */
...
gen BUS      = max(0,x3129)+max(0,x3124)-max(0,x3126)*(x3127==5)+max(0,x3121)*(x3122==1|x3222==6) ///
              ...
              +max(0,x3221)*(x3222==1|x3222==6)+max(0,x2723*(x2710==72))+max(0,x2740*(x2727==72)) ///
              ...
              +max(0,x3321)*(x3322==1|x3222==6)+max(0,x2723*(x2710==73))+max(0,x2740*(x2727==73)) ///
...
gen CALLDBT  = max(0,x3932)*(x7194==5)
...
gen EDUC     = 0
replace EDUC = 1 if /* RTORESP==1 & */ x5902!=1 /* No high school deg */
replace EDUC = 2 if /* RTORESP==1 & */ x5902==1 /* High school deg */
replace EDUC = 3 if /* RTORESP==1 & */ x5904==1 /* College deg */
